    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        p1 = rg.Point(intersection_center.x + width/2, intersection_center.y + letter_thickness/2)
        p2 = rg.Point(intersection_center.x - width/2, intersection_center.y - letter_thickness/2)
        p3 = rg.Point(intersection_center.x - letter_thickness/2, intersection_center.y - letter_thickness/2)
        p4 = rg.Point(intersection_center.x + letter_thickness/2, intersection_center.y - letter_thickness/2 + height)
    ...
    def move_by(self, dx, dy):
        ...
        # ---------------------------------------------------------------------
        # vul = self.v_rect.get_upper_left_corner()
        # vlr = self.v_rect.get_lower_right_corner()
        # p1 = rg.Point(vul.x + dx, vul.y + dy)
        # p2 = rg.Point(vlr.x + dx, vlr.y + dy)
        # self.v_rect = rg.Rectangle(p1, p2)
        # hul = self.h_rect.get_upper_left_corner()
        # hlr = self.h_rect.get_lower_right_corner()
        # p3 = rg.Point(hul.x + dx, hul.y + dy)
        # p4 = rg.Point(hlr.x + dx, hlr.y + dy)
        # self.h_rect = rg.Rectangle(p3, p4)
    ...
    def clone(self):
        ...
        #######################################################################
        new_center = self.h_rect.get_center()
        new_width = self.h_rect.get_width()
